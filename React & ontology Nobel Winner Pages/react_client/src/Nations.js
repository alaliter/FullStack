...
import NationsCard from './NationsCard';
import axios from 'axios';

const API = 'http://tinman.cs.gsu.edu:5001/nobel/nations';
...
class Nations extends Component {
  ...
  async handleClick(e) {
    // alert(e.target.id)
    const {data:winners} = await axios.get('http://tinman.cs.gsu.edu:5001/nobel/nations/'+e.target.id);
  ...
  renderInfo() {
    ...
    let Info_list = this.state.info.map(function(el) {
      return(
          <NationsCard info={el} handleClick={h}/>
      );
    });
...
export default withRouter(Nations);